...
pub async fn raw_consumer(
    ...
    let deliver_policy = match start {
        Some(start_sequence) => {
            // The deliver policy is fixed at creation; recreate to move it. A
            // missing consumer is the fresh-recovery case and benign, but any
            // *other* delete failure would leave the old policy in place and
            // silently replay from the wrong point — so it is fatal.
