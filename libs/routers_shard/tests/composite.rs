...
fn two_shard_setup() -> (MultiShardNetwork<OsmEntryId, OsmEdgeMetadata, QuadKey>, QuadKey, QuadKey)
{
...
fn composite_unifies_node_and_edge_counts() {
    ...
    assert_eq!(composite.shard_count(), 2, "should hold both shards: {a:?}, {b:?}");
...
fn composite_routes_across_shard_boundary() {
    ...
    let composite =
        MultiShardNetwork::new(vec![Arc::new(n_sw), Arc::new(n_ne)]);
...
fn empty_composite_routes_nothing() {
    let composite =
        MultiShardNetwork::<OsmEntryId, OsmEdgeMetadata, QuadKey>::empty();
    assert_eq!(composite.shard_count(), 0);
    assert_eq!(composite.num_nodes(), 0);
    assert!(composite.route_nodes(OsmEntryId::node(1), OsmEntryId::node(2)).is_none());
