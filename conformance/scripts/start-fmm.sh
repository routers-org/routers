...
# Uses the FastMapMatch algorithm — requires both the Sydney road network
# shapefile and the precomputed UBODT table.
# Run 'nix run .#setup-fmm' first to generate both.
set -euo pipefail
...
FMM_NETWORK="$CONFORMANCE_WORK/network/sydney-roads.shp"
FMM_UBODT="$CONFORMANCE_WORK/network/sydney-ubodt.bin"
